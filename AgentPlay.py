...
def AgentPlay(agent,env,brain_name,train_mode=False,counter_states=3,view_episodes=1,loop_breaker=False,*args,**kwargs):
    """
    Watch the agent play in Unity: 
            ...
            *args , **kwargs: passed to agent.act (besides state and eps) 
    """
    score_list = []
    loop_list=[]
    for i_episode in range(1, view_episodes+1): 
        ...
        while True:
            if hasattr(agent,'time'):
                if agent.time==True:
                    action = agent.act(state,eps,*args,time_step=time_step,**kwargs) 
            else:
                action = agent.act(state,eps,*args,**kwargs)
            # select an action
